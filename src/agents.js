...
(function (root) {
  ...
  class Brain {
    ...
    control(w, c, dt) {
      ...
      let reverse = false;
      if (Math.abs(angle) > 2.65 && toTarget < 7) {
        reverse = true;
        steer = clamp(-wrap(angle + Math.PI) * 2.5, -1, 1);
        desired = -Math.min(12, toTarget * 3);
      } else {
      ...
      if ((p.kind === 'space' || p.kind === 'support') && toTarget < 1.8) desired = 0;
    ...
    step(w, c, dt) {
      const v = this.ballAtPlan,
        unexpected = v && c.planAge > 0.1 && Math.hypot(w.ball.vx - v.vx, w.ball.vz - v.vz) > 7;
